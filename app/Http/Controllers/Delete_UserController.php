...
class Delete_UserController extends AppBaseController
{
    /** @var  UserRepository */
    ...
    {
        if ($request->ajax()) {
            return Datatables::of((new UserDataTable())->get($request->only(['filter_user']), 2, 1))->make(true);
        }
        $roles = Role::all()->pluck('name', 'id')->toArray();

        return view('delete_users.index')->with([
            'roles' => $roles,
        ]);
    }
    ...
    public function create()
    {
        ...
        return view('delete_users.create')->with(['roles' => $roles]);
    }
    ...
    public function show(User $user)
    {
        ...
        return view('delete_users.show')->with('user', $user);
    }
