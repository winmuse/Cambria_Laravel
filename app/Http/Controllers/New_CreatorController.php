...
class New_CreatorController extends AppBaseController
{
    /** @var  UserRepository */
    ...
    {
        if ($request->ajax()) {
            return Datatables::of((new UserDataTable())->get($request->only(['filter_user']), 0, 2))->make(true);
        }
        $roles = Role::all()->pluck('name', 'id')->toArray();

        return view('new_creators.index')->with([
            'roles' => $roles,
        ]);
    }
    ...
    public function create()
    {
        ...
        return view('new_creators.create')->with(['roles' => $roles]);
    }
    ...
    public function show(User $user)
    {
        ...
        return view('new_creators.show')->with('user', $user);
    }
