...
class Delete_CreatorController extends AppBaseController
{
    /** @var  UserRepository */
    ...
    {
        if ($request->ajax()) {
            return Datatables::of((new UserDataTable())->get($request->only(['filter_user']), 2, 2))->make(true);
        }
        $roles = Role::all()->pluck('name', 'id')->toArray();

        return view('delete_creators.index')->with([
            'roles' => $roles,
        ]);
    }
    ...
    public function create()
    {
        ...
        return view('delete_creators.create')->with(['roles' => $roles]);
    }
    ...
    public function show(User $user)
    {
        ...
        return view('delete_creators.show')->with('user', $user);
    }
    ...
    public function restoreDeRestoreUser(User $user)
    {
        $this->userRepository->restoreDeRestoreUser($user->id);
