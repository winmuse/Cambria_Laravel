...
use App\Models\Role;
use App\Models\User;
use App\Queries\CreatorDataTable;
...
class Creator_PaymentController extends AppBaseController
{
    /** @var  UserRepository */
    ...
    {
        if ($request->ajax()) {
            return Datatables::of((new CreatorDataTable())->get($request->only(['filter_user']),1,2))->make(true);
        }
        $roles = Role::all()->pluck('name', 'id')->toArray();

        return view('creator_payment.index')->with([
            'roles' => $roles,
        ]);
    }
    ...
    public function create()
    {
        ...
        return view('creator_payment.create')->with(['roles' => $roles]);
    }
    ...
    public function show(User $user)
    {
        ...
        return view('creator_payment.show')->with('user', $user);
    }
