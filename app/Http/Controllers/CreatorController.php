...
class CreatorController extends AppBaseController
{
    ...
    {
        if ($request->ajax()) {
            return Datatables::of((new UserDataTable())->get($request->only(['filter_user']), 1, 2))->make(true);
        }
        $roles = Role::all()->pluck('name', 'id')->toArray();

        return view('creators.index')->with([
            'roles' => $roles,
        ]);
    }
    ...
    public function create()
    {
        ...
        return view('creators.create')->with(['roles' => $roles]);
    }
    ...
    public function show(User $user)
    {
        ...
        return view('creators.show')->with('user', $user);
    }
    ...
    public function destroy($id)
    {
        ...
        $this->userRepository->deleteDeActiveUser($user->id);
