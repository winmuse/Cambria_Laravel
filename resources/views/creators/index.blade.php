@extends('layouts.app')
@section('title')
    {{ __('messages.current_creator_list') }}
@endsection
@section('page_css')
...
@section('content')
    ...
    @include('users.edit')
    @include('creators.templates.action_icons')
@endsection
@section('page_js')
...
@section('scripts')
    <script>
        let createUserUrl = "{{ route('creators.store') }}";
        let usersUrl = "{{ url('creators') }}/";
        let defaultImageAvatar = "{{ getDefaultAvatar() }}/";
    </script>
    <script src="{{ mix('assets/js/admin/creators/creator.js') }}"></script>
    <script src="{{ mix('assets/js/admin/creators/edit_user.js') }}"></script>
